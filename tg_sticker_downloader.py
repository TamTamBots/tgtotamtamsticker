...
class TGStickerDownloader:
    ...
    def get_sticker_pack_by_name(self, name) -> Optional[StickersSet]:
        ...
        stickers = [
            Sticker(file_id=sticker["file_id"],
                    emoji=sticker["emoji"],
                    file_bytes=b"") for sticker in res["result"]["stickers"]
        ]
        return StickersSet(res["result"]["name"], res["result"]["title"],
                           stickers)

    def get_sticker_file(self, file_id) -> TGFile:
        method = "getFile"
        res = self.api_request(method, {"file_id": file_id})["result"]
        return TGFile(res["file_id"], res["file_unique_id"], res["file_size"],
                      res["file_path"])
    ...
    def api_request(self, method: str, params: dict = None) -> dict:
        ...
        if resp.status_code == 200:
            ...
            raise TGStickerDownloaderException(
                f"telegram API error, status code: {resp.status_code}, text: {resp.text}"
            )
    ...
    def create_tamtam_zip(self, tg_set: StickersSet) -> "List[str]":
        ...
        with TemporaryDirectory(suffix=tg_set.name) as tmpdir:
            pool.starmap(
                self.proceed_sticker,
                [(x, tmpdir, tg_set.name, result_files)
                 for x in tg_set.stickers],
            )
            result_zip_names = []
            ...
            for part_n, part in enumerate(parts):
                ...
                with zipfile.ZipFile(zip_name, "w") as zf:
                    for sticker_file in part:
                        ...
                        zf.write(sticker_file,
                                 arcname=sticker_file.split("/")[-1])
                    result_zip_names.append(zip_name)
        return result_zip_names

    def proceed_sticker(self, sticker: Sticker, tmp_dir: str, pack_name: str,
                        result: list):
        ...
        if st_file.file_path.endswith("tgs"):
            ...
            sticker.file_bytes = img_convertor.convert_to_tt_format(
                sticker.file_bytes)
